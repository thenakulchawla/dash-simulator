...
namespace ns3 {
    ...
    void 
        DashMiner::MineBlock (void)
        {
                    ...
                    case STANDARD:
                        {
                            ...
                            else if (m_protocolType == XTHIN)
                            {
                                ...
                                m_nodeStats->invSentBytes += m_dashMessageHeader + m_countBytes + inv["inv"].Size()*m_inventorySizeBytes;
                                std::string temp = invInfo.GetString();

                                // NS_LOG_INFO("Sending xthin block with trimmed inverted header: " <<  temp.substr(0,100));
                            }
            ...
            // if (m_targetNumberOfBlocks * 10 <= m_blockchain.GetTotalBlocks()) 
            // {
