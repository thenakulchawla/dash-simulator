...
namespace ns3 {
    ...
    void 
        DashMiner::MineBlock (void)
        {
            ...
            //fine tune the transaction data to get more accurate data
            //transactions based on fixed blcok size or distribution
            ...
            if (m_fixedBlockSize > 0)
            {
                ...
                {
                    m_nextBlockSize = m_fixedBlockSize * .009; //approximation in the most adversarial case
                    transactionCount = (m_nextBlockSize - m_blockHeadersSizeBytes -8 -1 ) / shortTransactionSizeInBytes;
                ...
                {
                    //std::cout<<"Fixed Block Size : " <<m_fixedBlockSize<<"\n";
                    m_nextBlockSize = m_fixedBlockSize;
                    while (blockSize <= m_fixedBlockSize)
                    {
            ...
            else
            {
                ...
                m_transactionCountDistribution = std::piecewise_constant_distribution<double> (iCount.begin(),iCount.end(), wCount.begin());
                transactionCount = (int) m_transactionCountDistribution(m_generator);
                int i=0;
            ...
            transactionsPerSec = ((double) transactionCount)/m_averageBlockGenIntervalSeconds;
