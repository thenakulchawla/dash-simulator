...
/**
 * The dash message types that have been implemented.
 */
enum Messages
{
  INV,              //0
...
public:
  ...
  DashChunk& operator= (const DashChunk &chunkSource); //Assignment Constructor
  
  friend bool operator== (const DashChunk &chunk, const DashChunk &chunk2);
  friend bool operator< (const DashChunk &chunk, const DashChunk &chunk2);
  friend std::ostream& operator<< (std::ostream &out, const DashChunk &chunk);
