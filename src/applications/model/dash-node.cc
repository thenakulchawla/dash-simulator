/**
 * This file contains the definitions of the functions declared in dash-node.h
 */
...
NS_LOG_COMPONENT_DEFINE ("DashNode");

NS_OBJECT_ENSURE_REGISTERED (DashNode);

TypeId 
DashNode::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::DashNode")
    .SetParent<Application> ()
    .SetGroupName("Applications")
    .AddConstructor<DashNode> ()
    .AddAttribute ("Local",
                   "The Address on which to Bind the rx socket.",
                   AddressValue (),
                   MakeAddressAccessor (&DashNode::m_local),
                   MakeAddressChecker ())
    .AddAttribute ("Protocol",
                   "The type id of the protocol to use for the rx socket.",
                   TypeIdValue (UdpSocketFactory::GetTypeId ()),
                   MakeTypeIdAccessor (&DashNode::m_tid),
                   MakeTypeIdChecker ())
    .AddAttribute ("BlockTorrent",
                   "Enable the BlockTorrent protocol",
                   BooleanValue (false),
                   MakeBooleanAccessor (&DashNode::m_blockTorrent),
                   MakeBooleanChecker ())
    .AddAttribute ("SPV",
                   "Enable SPV Mechanism",
                   BooleanValue (false),
                   MakeBooleanAccessor (&DashNode::m_spv),
                   MakeBooleanChecker ())
    .AddAttribute ("InvTimeoutMinutes", 
				   "The timeout of inv messages in minutes",
                   TimeValue (Minutes (20)),
                   MakeTimeAccessor (&DashNode::m_invTimeoutMinutes),
                   MakeTimeChecker())
    .AddAttribute ("ChunkSize", 
				   "The fixed size of the block chunk",
                   UintegerValue (100000),
                   MakeUintegerAccessor (&DashNode::m_chunkSize),
                   MakeUintegerChecker<uint32_t> ())
    .AddTraceSource ("Rx",
                  "A packet has been received",
                  MakeTraceSourceAccessor (&DashNode::m_rxTrace),
                  "ns3::Packet::AddressTracedCallback")
    .AddAttribute("raptor",
            "Raptor enabled",
            BooleanValue(false),
            MakeBooleanAccessor (&DashNode::m_raptor),
            MakeBooleanChecker())
...
DashNode::DashNode (void) : m_dashPort (9999), m_secondsPerMin(60), m_isMiner (false), m_countBytes (4), m_dashMessageHeader (90),
                                  m_inventorySizeBytes (36), m_getHeadersSizeBytes (72), m_headersSizeBytes (81), m_blockHeadersSizeBytes (81), m_averageTransactionSize(522), m_transactionIndexSize(2)
{
  m_socket = 0;
...
DashNode::~DashNode(void)
{
  // NS_LOG_FUNCTION (this);
}

Ptr<Socket>
DashNode::GetListeningSocket (void) const
{
  // NS_LOG_FUNCTION (this);
  return m_socket;
}
...
DashNode::GetPeersAddresses (void) const
{
  // NS_LOG_FUNCTION (this);
...
void 
DashNode::SetPeersAddresses (const std::vector<Ipv4Address> &peers)
{
  // NS_LOG_FUNCTION (this);
...
void 
DashNode::SetPeersDownloadSpeeds (const std::map<Ipv4Address, double> &peersDownloadSpeeds)
{
  // NS_LOG_FUNCTION (this);
...
void 
DashNode::SetPeersUploadSpeeds (const std::map<Ipv4Address, double> &peersUploadSpeeds)
{
  // NS_LOG_FUNCTION (this);
...
void 
DashNode::SetNodeInternetSpeeds (const nodeInternetSpeeds &internetSpeeds)
{
  // NS_LOG_FUNCTION (this);

	if(m_nodeType == MASTER_NODE)
	{
		m_downloadSpeed = internetSpeeds.downloadSpeed * 1000000000 / 8;
		m_uploadSpeed = internetSpeeds.uploadSpeed * 1000000000 / 8 ; 
	}
	else {
...
void 
DashNode::SetNodeStats (nodeStatistics *nodeStats)
{
  // NS_LOG_FUNCTION (this);
...
void 
DashNode::SetProtocolType (enum ProtocolType protocolType)
{
  // NS_LOG_FUNCTION (this);
...
void 
DashNode::SetNodeType (enum NodeType nodeType)
{
  // NS_LOG_FUNCTION (this);
  m_nodeType = nodeType;
}
...
void 
DashNode::StartApplication ()    // Called at time specified by Start
{
  ...
  m_socket->SetRecvCallback (MakeCallback (&DashNode::HandleRead, this));
  m_socket->SetAcceptCallback (
    MakeNullCallback<bool, Ptr<Socket>, const Address &> (),
    MakeCallback (&DashNode::HandleAccept, this));
  m_socket->SetCloseCallbacks (
    MakeCallback (&DashNode::HandlePeerClose, this),
    MakeCallback (&DashNode::HandlePeerError, this));
  ...
  for (std::vector<Ipv4Address>::const_iterator i = m_peersAddresses.begin(); i != m_peersAddresses.end(); ++i)
  {
    ...
    m_peersSockets[*i]->Connect (InetSocketAddress (*i, m_dashPort));
  }
      ...
      if (InetSocketAddress::IsMatchingType (from))
      {
        ...
        while ((pos = totalReceivedData.find(delimiter)) != std::string::npos) 
        {
          parsedPacket = totalReceivedData.substr(0, pos);
					...
					// NS_LOG_INFO("parsed packet is : " << parsedPacket << std::endl);
          ...
          switch (d["message"].GetInt())
          {
            ...
            case EXT_INV:
            {
              ...
              m_nodeStats->extInvReceivedBytes += m_dashMessageHeader + m_countBytes + d["inv"].Size()*m_inventorySizeBytes;
              ...
              {  
                ...
                {
                  // NS_LOG_INFO("EXT_INV: Dash node " << GetNode ()->GetId () 
                  //             << " has already received the block with height = " 
                  //             << height << " and minerId = " << minerId);				  
                }
                else
                ...
                /*  std::cout << "candidateChunks = ";
                    ...
                    if (candidateChunks.size() > 0)
                    {
                      ...
                      timeout = Simulator::Schedule (Minutes(m_invTimeoutMinutes.GetMinutes() / ceil(blockSize/static_cast<double>(m_chunkSize))),
                                                     &DashNode::ChunkTimeoutExpired, this, chunk.str());
            ...
            case GET_HEADERS:
            {
              ...
              m_nodeStats->getHeadersReceivedBytes += m_dashMessageHeader + m_getHeadersSizeBytes;
              ...
              {  
                ...
                {
                  NS_LOG_INFO("GET_HEADERS: Dash node " << GetNode ()->GetId () 
            ...
            case EXT_GET_HEADERS:
            {
              ...
              m_nodeStats->extGetHeadersReceivedBytes += m_dashMessageHeader + m_getHeadersSizeBytes;
              ...
              {  
                ...
                if (m_blockchain.HasBlock(height, minerId) || m_blockchain.IsOrphan(height, minerId))
                {
                  // NS_LOG_INFO("EXT_GET_HEADERS: Dash node " << GetNode ()->GetId () 
                  //             << " has the block with height = " 
                  //             << height << " and minerId = " << minerId);
                  Block newBlock (m_blockchain.ReturnBlock (height, minerId));
                  requestHeaders.push_back(newBlock); 
                ...
                for (j =0; j <d["blocks"].Size(); ++j)
                {
                    ...
                    { 
                        ...
                        for (block_it = requestBlocks.begin(); block_it < requestBlocks.end(); block_it++) 
                        {
                            ...
                            int symbolSize = static_cast<double>(block_it->GetBlockSizeBytes()/10000);
                            value = symbolSize;
            ...
            case GET_DATA:
            {
              ...
              if (!requestBlocks.empty())
              {
                ...
                Simulator::Schedule (Seconds(eventTime), &DashNode::SendBlock, this, packet, from);
                Simulator::Schedule (Seconds(eventTime + sendTime), &DashNode::RemoveSendTime, this);
            ...
            case EXT_GET_DATA:
            {
              ...
              m_nodeStats->extGetDataReceivedBytes += m_dashMessageHeader + m_countBytes + d["chunks"].Size()*m_inventorySizeBytes;
              ...
              {  
                ...
                if (m_blockchain.HasBlock(height, minerId) || m_blockchain.IsOrphan(height, minerId) || ReceivedButNotValidated(blockHash))
                {
                  // NS_LOG_INFO("EXT_GET_DATA: Dash node " << GetNode ()->GetId () 
                  // << " has already received the block with height = " 
                  // << height << " and minerId = " << minerId);
...
/*                 std::cout << "Node " << GetNode()->GetId() << "-" << InetSocketAddress::ConvertFrom(from).GetIpv4 () 
                ...
                Simulator::Schedule (Seconds(eventTime), &DashNode::SendChunk, this, packet, from);
                Simulator::Schedule (Seconds(eventTime + sendTime), &DashNode::RemoveSendTime, this);
              }
              break;
            }
            case HEADERS:
            {
              ...
              m_nodeStats->headersReceivedBytes += m_dashMessageHeader + m_countBytes + d["blocks"].Size()*m_headersSizeBytes;
              ...
              {  
                ...
                if(m_protocolType == SENDHEADERS && !m_blockchain.HasBlock(height, minerId) && !m_blockchain.IsOrphan(height, minerId) && !ReceivedButNotValidated(blockHash))
                {
                  ...
                  {
                    NS_LOG_INFO("HEADERS: Dash node " << GetNode ()->GetId ()
                                 << " has not requested the block yet");
                ...
                {				  
                  ...
                  if (m_invTimeouts.find(parentBlockHash) == m_invTimeouts.end())
                  {
                      ...
                      (m_protocolType == SENDHEADERS && std::find(requestBlocks.begin(), requestBlocks.end(), parentBlockHash) == requestBlocks.end()))
                    {
                      ...
                      timeout = Simulator::Schedule (m_invTimeoutMinutes, &DashNode::InvTimeoutExpired, this, parentBlockHash);
            ...
            case EXT_HEADERS:
            {
              ...
              m_nodeStats->extHeadersReceivedBytes += m_dashMessageHeader + m_countBytes + d["blocks"].Size()*m_headersSizeBytes;
              ...
              {  
                ...
                if(!m_blockchain.HasBlock(height, minerId) && !m_blockchain.IsOrphan(height, minerId) && !ReceivedButNotValidated(blockHash))
                {
                  ...
                  // NS_LOG_INFO("EXT_HEADERS: Dash node " << GetNode ()->GetId () 
                  //             << " does not have the block with height = " 
                  //             << height << " and minerId = " << minerId);
            ...
            case BLOCK:
            {
              ...
              blockMessageSize += m_dashMessageHeader;

              for (int j=0; j<d["blocks"].Size(); j++)
              {  
                if (blockType == "block")
                  blockMessageSize += d["blocks"][j]["size"].GetInt();
                // TODO: Nakul think about this later, you don't have to deal with this now.
                else if (blockType == "compressed-block")
                {
                  int transactionCount = static_cast<int>((d["blocks"][j]["size"].GetInt() - m_blockHeadersSizeBytes)/ m_averageTransactionSize);
              ...
              else if (blockType == "compressed-block")
              {
                ...
                Simulator::Schedule (Seconds(eventTime), &DashNode::ReceivedBlockMessage, this, help, from);
                Simulator::Schedule (Seconds(receiveTime), &DashNode::RemoveCompressedBlockReceiveTime, this);
              }
            ...
            case CHUNK:
            {
              ...
              chunkMessageSize += m_dashMessageHeader;
              ...
              Simulator::Schedule (Seconds(eventTime), &DashNode::ReceivedChunkMessage, this, help, from);
              Simulator::Schedule (Seconds(receiveTime), &DashNode::RemoveReceiveTime, this);
            ...
            case RAPTORCODE:
            {
                ...
                for (j=0;j<d["raptors"].Size();++j)
                {
                    double symbolSize = d["raptors"][j]["symbolSize"].GetDouble();
                    double receivedBytes = symbolSize;
                    m_nodeStats->raptorReceivedBytes += receivedBytes; 

                    int blockSize= d["raptors"][j]["blockSize"].GetInt();

                    if (m_receiveBlockTimes.size() == 0 || Simulator::Now ().GetSeconds() >  m_receiveBlockTimes.back())
                    {
                        receiveTime = symbolSize / m_downloadSpeed; 
                        eventTime = symbolSize / minSpeed;
                    }
                    else
                    {
                        receiveTime = symbolSize / m_downloadSpeed + m_receiveBlockTimes.back() - Simulator::Now ().GetSeconds(); 
                        eventTime = symbolSize / minSpeed + m_receiveBlockTimes.back() - Simulator::Now ().GetSeconds(); 
                    }
                    m_receiveBlockTimes.push_back(Simulator::Now ().GetSeconds() + receiveTime);
...
void 
DashNode::ReceivedBlockMessage(std::string &blockInfo, Address &from) 
{
  NS_LOG_FUNCTION (this);
...
void
DashNode::ReceivedRaptorCode(std::string &raptorInfo, Address &from)
{
    NS_LOG_FUNCTION(this);
  ...
  {
    int parentHeight = d["raptors"][j]["height"].GetInt() - 1;
    int parentMinerId = d["raptors"][j]["parentBlockMinerId"].GetInt();
    int height = d["raptors"][j]["height"].GetInt();
    int minerId = d["raptors"][j]["minerId"].GetInt();
    ...
    std::string blockHash = d["raptors"][j]["hash"].GetString();

    if (m_symbolsReceived.find(blockHash) != m_symbolsReceived.end())
    {
        // NS_LOG_INFO("blockhash found");
        m_symbolsReceived[blockHash] += d["raptors"][j]["symbolCount"].GetInt();
        ...
        if (m_symbolsReceived[blockHash] >= totalRequired)
        {
            ...
            if (!m_blockchain.HasBlock(parentHeight, parentMinerId) && !m_blockchain.IsOrphan(parentHeight, parentMinerId)
                    && !ReceivedButNotValidated(parentBlockHash) && !OnlyHeadersReceived(parentBlockHash))
            {				  
                NS_LOG_INFO("The Block with height = " << d["raptors"][j]["height"].GetInt() 
                        << " and minerId = " << d["raptors"][j]["minerId"].GetInt() 
                        << " is an orphan, so it will be discarded\n");
            }
            else
            {
    ...
    {
        // NS_LOG_INFO("blockhash not found");
        m_symbolsReceived.insert({blockHash, d["raptors"][j]["symbolCount"].GetInt()});
        double symbolSize =d["raptors"][0]["symbolSize"].GetDouble() ;
        ...
        for (std::vector<Ipv4Address>::const_iterator i = m_peersAddresses.begin(); i != m_peersAddresses.end(); ++i)
        {
            ...
            Simulator::Schedule (Seconds(eventTime), &DashNode::SendRaptorSymbol, this, raptorInfo, m_peersSockets[*i]);
            Simulator::Schedule (Seconds(eventTime + sendTime), &DashNode::RemoveSendTime, this);
...
void 
DashNode::ReceivedChunkMessage(std::string &chunkInfo, Address &from) 
{
  NS_LOG_FUNCTION (this);
  ...
  //             << "s dash node " << GetNode ()->GetId () << " received a  message " << chunkInfo);
			
  //m_receiveBlockTimes.erase(m_receiveBlockTimes.begin());	

  std::vector<std::string>                 getDataMessages;
  std::map<DashChunk, std::vector<int>>    chunkMessages;
  int totalChunkMessageSize = 0;
    ...
    if (!m_blockchain.HasBlock(height, minerId) && !m_blockchain.IsOrphan(height, minerId) && !ReceivedButNotValidated(blockHash))
    {
      ...
      if(std::find(m_receivedChunks[blockHash].begin(), m_receivedChunks[blockHash].end(), chunkId) == m_receivedChunks[blockHash].end())
      {
        ...
        else
        {
          ...
          {
            int randomIndex = rand() % candidateChunks.size();
            // NS_LOG_INFO("ReceivedChunkMessage: Dash node " << GetNode ()->GetId ()
            //             << " will request the chunk with index = " << randomIndex << " and value = " << candidateChunks[randomIndex]);	
            m_queueChunks[blockHash].erase(std::remove(m_queueChunks[blockHash].begin(),
                                                       m_queueChunks[blockHash].end(), candidateChunks[randomIndex]),
                                                       m_queueChunks[blockHash].end());
...
DashNode::ReceiveBlock(const Block &newBlock) 
{
  ...
  {
    NS_LOG_INFO ("ReceiveBlock: Dash node " << GetNode ()->GetId () << " has NOT added this block in the m_blockchain: " << newBlock );
...
DashNode::ReceivedLastRaptorSymbol(const Block &newBlock) 
{
  ...
  if (m_blockchain.HasBlock(newBlock) || m_blockchain.IsOrphan(newBlock) || ReceivedButNotValidated(blockHash))
  {
    NS_LOG_INFO ("ReceivedLastRaptorSymbol: Dash node " << GetNode ()->GetId () << " has already added this block in the m_blockchain: " << newBlock);
  }
  else
  {
    NS_LOG_INFO ("ReceivedLastRaptorSymbol: Dash node " << GetNode ()->GetId () << " has NOT added this block in the m_blockchain: " << newBlock);
...
void 
DashNode::ReceivedLastChunk(const Block &newBlock) 
{
  NS_LOG_FUNCTION (this);
  ...
  if (m_blockchain.HasBlock(newBlock) || m_blockchain.IsOrphan(newBlock) || ReceivedButNotValidated(blockHash))
  {
    NS_LOG_INFO ("ReceivedLastChunk: Dash node " << GetNode ()->GetId () << " has already added this block in the m_blockchain: " << newBlock);
  }
  else
  {
    NS_LOG_INFO ("ReceivedLastChunk: Dash node " << GetNode ()->GetId () << " has NOT added this block in the m_blockchain: " << newBlock);
...
void 
DashNode::SendBlock(std::string packetInfo, Address& from) 
{
  NS_LOG_FUNCTION (this);
...
void 
DashNode::ReceivedHigherBlock(const Block &newBlock) 
{
    NS_LOG_FUNCTION(this);
  NS_LOG_INFO("ReceivedHigherBlock: Dash node " << GetNode ()->GetId () << " added a new block in the m_blockchain with higher height: " << newBlock);
}


void 
DashNode::ValidateBlock(const Block &newBlock) 
{
    NS_LOG_FUNCTION(this);
  ...
  else 
  {
    ...
    Simulator::Schedule ( Seconds(validationTime), &DashNode::AfterBlockValidation, this, newBlock);
...
void 
DashNode::AfterBlockValidation(const Block &newBlock) 
{
  NS_LOG_FUNCTION (this); 
...
void 
DashNode::ValidateOrphanChildren(const Block &newBlock) 
{
  NS_LOG_FUNCTION (this);
...
void 
DashNode::AdvertiseNewBlock (const Block &newBlock) 
{
  NS_LOG_FUNCTION (this);
...
void 
DashNode::AdvertiseFullBlock (const Block &newBlock) 
{
  NS_LOG_FUNCTION (this);
  ...
  for (std::vector<Ipv4Address>::const_iterator i = m_peersAddresses.begin(); i != m_peersAddresses.end(); ++i)
  {
    ...
    if (m_protocolType == STANDARD_PROTOCOL)
    {
      m_nodeStats->extInvSentBytes += m_dashMessageHeader + m_countBytes + d["inv"].Size()*m_inventorySizeBytes;
    ...
    {
      m_nodeStats->extHeadersSentBytes += m_dashMessageHeader + m_countBytes + d["blocks"].Size()*m_headersSizeBytes;
      for (int j=0; j<d["blocks"].Size(); j++)
      {
...
void 
DashNode::AdvertiseFirstChunk (const Block &newBlock) 
{
  NS_LOG_FUNCTION (this);
  ...
  for (std::vector<Ipv4Address>::const_iterator i = m_peersAddresses.begin(); i != m_peersAddresses.end(); ++i)
  {
    if ( *i != newBlock.GetReceivedFromIpv4 () )
    {
      ...
      NS_LOG_INFO ("AdvertiseFirstChunk: At time " << Simulator::Now ().GetSeconds ()
                   << "s dash node " << GetNode ()->GetId () << " advertised a new chunk: " 
                   << newBlock << " to " << *i);
    }
  }
...
DashNode::SendMessage(enum Messages receivedMessage,  enum Messages responseMessage, rapidjson::Document &d, Ptr<Socket> outgoingSocket)
{
  ...
  switch (d["message"].GetInt()) 
  {
    ...
    case GET_HEADERS:
    {
      m_nodeStats->getHeadersSentBytes += m_dashMessageHeader + m_getHeadersSizeBytes;
      break;
    }
    case EXT_GET_HEADERS:
    {
      m_nodeStats->extGetHeadersSentBytes += m_dashMessageHeader + m_getHeadersSizeBytes;
      break;
    }
    case HEADERS:
    {
    ...
    case BLOCK:
    {
      ...
      m_nodeStats->blockSentBytes += m_dashMessageHeader;
      break;
    }
    case CHUNK:
    {
      ...
      m_nodeStats->chunkSentBytes += m_dashMessageHeader;
      break;
    }
    case GET_DATA:
    {
      m_nodeStats->getDataSentBytes += m_dashMessageHeader + m_countBytes + d["blocks"].Size()*m_inventorySizeBytes;
      break;
    }
    case EXT_GET_DATA:
    {
      m_nodeStats->extGetDataSentBytes += m_dashMessageHeader + m_countBytes + d["chunks"].Size()*m_inventorySizeBytes;
      for (int j=0; j<d["chunks"].Size(); j++)
      {
...
DashNode::SendMessage(enum Messages receivedMessage,  enum Messages responseMessage, rapidjson::Document &d, Address &outgoingAddress)
{
  ...
  if (it == m_peersSockets.end()) //Create the socket if it doesn't exist
  {
    ...
    m_peersSockets[outgoingIpv4Address]->Connect (InetSocketAddress (outgoingIpv4Address, m_dashPort));
  }
...
void 
DashNode::PrintOnlyHeadersReceived()
{
  // NS_LOG_FUNCTION (this);
...
void
DashNode::InvTimeoutExpired(std::string blockHash)
{
  // NS_LOG_FUNCTION (this);
...
void
DashNode::ChunkTimeoutExpired(std::string chunk)
{
  // NS_LOG_FUNCTION (this);
...
bool 
DashNode::ReceivedButNotValidated (std::string blockHash)
{
  // NS_LOG_FUNCTION (this);
...
bool 
DashNode::OnlyHeadersReceived (std::string blockHash)
{
  // NS_LOG_FUNCTION (this);
...
bool 
DashNode::HasChunk (std::string blockHash, int chunk)
{
  // NS_LOG_FUNCTION (this);
...
void 
DashNode::RemoveCompressedBlockSendTime ()
{
  // NS_LOG_FUNCTION (this);
...
void 
DashNode::RemoveCompressedBlockReceiveTime ()
{
  // NS_LOG_FUNCTION (this);
...
void 
DashNode::HandlePeerClose (Ptr<Socket> socket)
{
  // NS_LOG_FUNCTION (this << socket);
}
 
void DashNode::HandlePeerError (Ptr<Socket> socket)
{
  // NS_LOG_FUNCTION (this << socket);
}
...
void 
DashNode::HandleAccept (Ptr<Socket> s, const Address& from)
{
  // NS_LOG_FUNCTION (this << s << from);
  s->SetRecvCallback (MakeCallback (&DashNode::HandleRead, this));
}
