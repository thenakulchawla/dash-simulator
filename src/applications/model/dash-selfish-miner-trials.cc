...
NS_LOG_COMPONENT_DEFINE ("DashSelfishMinerTrials");

NS_OBJECT_ENSURE_REGISTERED (DashSelfishMinerTrials);

TypeId 
DashSelfishMinerTrials::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::DashSelfishMinerTrials")
    .SetParent<Application> ()
    .SetGroupName("Applications")
    .AddConstructor<DashSelfishMinerTrials> ()
    .AddAttribute ("Local",
                   "The Address on which to Bind the rx socket.",
                   AddressValue (),
                   MakeAddressAccessor (&DashSelfishMinerTrials::m_local),
                   MakeAddressChecker ())
    .AddAttribute ("Protocol",
                   "The type id of the protocol to use for the rx socket.",
                   TypeIdValue (UdpSocketFactory::GetTypeId ()),
                   MakeTypeIdAccessor (&DashSelfishMinerTrials::m_tid),
                   MakeTypeIdChecker ())
    .AddAttribute ("NumberOfMiners", 
				   "The number of miners",
                   UintegerValue (16),
                   MakeUintegerAccessor (&DashSelfishMinerTrials::m_noMiners),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("FixedBlockSize", 
				   "The fixed size of the block",
                   UintegerValue (0),
                   MakeUintegerAccessor (&DashSelfishMinerTrials::m_fixedBlockSize),
                   MakeUintegerChecker<uint32_t> ())				   
    .AddAttribute ("FixedBlockIntervalGeneration", 
                   "The fixed time to wait between two consecutive block generations",
                   DoubleValue (0),
                   MakeDoubleAccessor (&DashSelfishMinerTrials::m_fixedBlockTimeGeneration),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("InvTimeoutMinutes", 
				   "The timeout of inv messages in minutes",
                   TimeValue (Minutes (20)),
                   MakeTimeAccessor (&DashSelfishMinerTrials::m_invTimeoutMinutes),
                   MakeTimeChecker())
    .AddAttribute ("HashRate", 
				   "The hash rate of the selfish miner",
                   DoubleValue (0.2),
                   MakeDoubleAccessor (&DashSelfishMinerTrials::m_hashRate),
                   MakeDoubleChecker<double> ())	
    .AddAttribute ("BlockGenBinSize", 
				   "The block generation bin size",
                   DoubleValue (-1),
                   MakeDoubleAccessor (&DashSelfishMinerTrials::m_blockGenBinSize),
                   MakeDoubleChecker<double> ())	
    .AddAttribute ("BlockGenParameter", 
				   "The block generation distribution parameter",
                   DoubleValue (-1),
                   MakeDoubleAccessor (&DashSelfishMinerTrials::m_blockGenParameter),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("AverageBlockGenIntervalSeconds", 
				   "The average block generation interval we aim at (in seconds)",
                   DoubleValue (10*60),
                   MakeDoubleAccessor (&DashSelfishMinerTrials::m_averageBlockGenIntervalSeconds),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("SecureBlocks", 
				   "The number of blocks required for the secure confirmation of the transactions",
                   UintegerValue (6),
                   MakeUintegerAccessor (&DashSelfishMinerTrials::m_secureBlocks),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("AdvertiseBlocks", 
				   "Choose whether the attacker will advertise his generated blocks",
                   UintegerValue (0),
                   MakeUintegerAccessor (&DashSelfishMinerTrials::m_advertiseBlocks),
                   MakeUintegerChecker<uint32_t> ())
    .AddTraceSource ("Rx",
                     "A packet has been received",
                     MakeTraceSourceAccessor (&DashSelfishMinerTrials::m_rxTrace),
...
DashSelfishMinerTrials::DashSelfishMinerTrials () : DashMiner(), m_attackFinished(false), m_winningStreak(0), m_trials(1)
{
  NS_LOG_FUNCTION (this);
}


DashSelfishMinerTrials::~DashSelfishMinerTrials(void)
{
  NS_LOG_FUNCTION (this);
}
...
void 
DashSelfishMinerTrials::StartApplication ()    // Called at time specified by Start
{
...
void 
DashSelfishMinerTrials::StopApplication ()
{
...
void 
DashSelfishMinerTrials::DoDispose (void)
{
  NS_LOG_FUNCTION (this);
  DashMiner::DoDispose ();
...
void 
DashSelfishMinerTrials::MineBlock (void)  
{
  NS_LOG_FUNCTION (this);
...
/*   stringStream.clear();
  ...
  if (m_fixedBlockSize > 0)
    ...
    //                  m_averageBlockGenIntervalSeconds / m_realAverageBlockGenIntervalSeconds;	// The block size is linearly dependent on the averageBlockGenIntervalSeconds
		...
		m_nextBlockSize = blockSize * m_averageBlockGenIntervalSeconds / m_realAverageBlockGenIntervalSeconds;
...
/* 	  //Send large packet
  ...
  //             << "s dash selfish miner " << GetNode ()->GetId () 
...
void 
DashSelfishMinerTrials::ReceivedHigherBlock(const Block &newBlock) 
{
  NS_LOG_FUNCTION (this);
