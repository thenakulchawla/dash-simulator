...
NS_LOG_COMPONENT_DEFINE ("DashSelfishMiner");

NS_OBJECT_ENSURE_REGISTERED (DashSelfishMiner);

TypeId 
DashSelfishMiner::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::DashSelfishMiner")
    .SetParent<Application> ()
    .SetGroupName("Applications")
    .AddConstructor<DashSelfishMiner> ()
    .AddAttribute ("Local",
                   "The Address on which to Bind the rx socket.",
                   AddressValue (),
                   MakeAddressAccessor (&DashSelfishMiner::m_local),
                   MakeAddressChecker ())
    .AddAttribute ("Protocol",
                   "The type id of the protocol to use for the rx socket.",
                   TypeIdValue (UdpSocketFactory::GetTypeId ()),
                   MakeTypeIdAccessor (&DashSelfishMiner::m_tid),
                   MakeTypeIdChecker ())
    .AddAttribute ("NumberOfMiners", 
				   "The number of miners",
                   UintegerValue (16),
                   MakeUintegerAccessor (&DashSelfishMiner::m_noMiners),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("FixedBlockSize", 
				   "The fixed size of the block",
                   UintegerValue (0),
                   MakeUintegerAccessor (&DashSelfishMiner::m_fixedBlockSize),
                   MakeUintegerChecker<uint32_t> ())				   
    .AddAttribute ("FixedBlockIntervalGeneration", 
                   "The fixed time to wait between two consecutive block generations",
                   DoubleValue (0),
                   MakeDoubleAccessor (&DashSelfishMiner::m_fixedBlockTimeGeneration),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("InvTimeoutMinutes", 
				   "The timeout of inv messages in minutes",
                   TimeValue (Minutes (20)),
                   MakeTimeAccessor (&DashSelfishMiner::m_invTimeoutMinutes),
                   MakeTimeChecker())
    .AddAttribute ("HashRate", 
				   "The hash rate of the selfish miner",
                   DoubleValue (0.2),
                   MakeDoubleAccessor (&DashSelfishMiner::m_hashRate),
                   MakeDoubleChecker<double> ())	
    .AddAttribute ("BlockGenBinSize", 
				   "The block generation bin size",
                   DoubleValue (-1),
                   MakeDoubleAccessor (&DashSelfishMiner::m_blockGenBinSize),
                   MakeDoubleChecker<double> ())	
    .AddAttribute ("BlockGenParameter", 
				   "The block generation distribution parameter",
                   DoubleValue (-1),
                   MakeDoubleAccessor (&DashSelfishMiner::m_blockGenParameter),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("AverageBlockGenIntervalSeconds", 
				   "The average block generation interval we aim at (in seconds)",
                   DoubleValue (10*60),
                   MakeDoubleAccessor (&DashSelfishMiner::m_averageBlockGenIntervalSeconds),
                   MakeDoubleChecker<double> ())
    .AddTraceSource ("Rx",
                     "A packet has been received",
                     MakeTraceSourceAccessor (&DashSelfishMiner::m_rxTrace),
...
DashSelfishMiner::~DashSelfishMiner(void)
{
  NS_LOG_FUNCTION (this);
}
...
void 
DashSelfishMiner::StartApplication ()    // Called at time specified by Start
{
...
void 
DashSelfishMiner::StopApplication ()
{
...
void 
DashSelfishMiner::DoDispose (void)
{
  NS_LOG_FUNCTION (this);
  DashMiner::DoDispose ();
...
void 
DashSelfishMiner::MineBlock (void)  
{
  NS_LOG_FUNCTION (this);
...
void 
DashSelfishMiner::ReceivedHigherBlock(const Block &newBlock) 
{
  NS_LOG_FUNCTION (this);
  NS_LOG_WARN("Dash selfish miner "<< GetNode ()->GetId () << " added a new block in the m_blockchain with higher height: " << newBlock);
...
void 
DashSelfishMiner::ReceiveBlock(const Block &newBlock) 
{
  NS_LOG_FUNCTION (this);
  NS_LOG_INFO ("DashSelfishMiner ReceiveBlock: At time " << Simulator::Now ().GetSeconds ()
  ...
  {
    NS_LOG_INFO ("DashSelfishMiner ReceiveBlock: Dash node " << GetNode ()->GetId () << " has already added this block in the m_blockchain: " << newBlock);
  ...
  {
    NS_LOG_INFO ("DashSelfishMiner ReceiveBlock: Dash node " << GetNode ()->GetId () << " has NOT added this block in the m_blockchain: " << newBlock);
...
void 
DashSelfishMiner::ReleaseChain(std::vector<Block> blocks)
{
  NS_LOG_FUNCTION (this);
  ...
  for (std::vector<Ipv4Address>::const_iterator i = m_peersAddresses.begin(); i != m_peersAddresses.end(); ++i, ++count)
  {
    ...
    switch(m_blockBroadcastType)				  
    {
      ...
      case UNSOLICITED:
      {
        ...
        m_nodeStats->blockSentBytes += m_dashMessageHeader + blockMessageSize;
...
/*                 std::cout << "Node " << GetNode()->GetId() << "-" << InetSocketAddress::ConvertFrom(from).GetIpv4 () 
        ...
        Simulator::Schedule (Seconds(eventTime), &DashSelfishMiner::SendBlock, this, packet, m_peersSockets[*i]);
        Simulator::Schedule (Seconds(eventTime + sendTime), &DashSelfishMiner::RemoveSendTime, this);
...
/* 				std::cout << "Node " << GetNode()->GetId() << "-" << *i 
          ...
          std::string packet = blockInfo.GetString();
          Simulator::Schedule (Seconds(sendTime), &DashSelfishMiner::SendBlock, this, packet, m_peersSockets[*i]);
          Simulator::Schedule (Seconds(eventTime + sendTime), &DashSelfishMiner::RemoveCompressedBlockSendTime, this);
...
enum Action 
DashSelfishMiner::ReadActionMatrix(enum ForkType f, int la, int lh)
{
  NS_LOG_FUNCTION (this);
