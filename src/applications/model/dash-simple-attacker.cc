...
NS_LOG_COMPONENT_DEFINE ("DashSimpleAttacker");

NS_OBJECT_ENSURE_REGISTERED (DashSimpleAttacker);

TypeId 
DashSimpleAttacker::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::DashSimpleAttacker")
    .SetParent<Application> ()
    .SetGroupName("Applications")
    .AddConstructor<DashSimpleAttacker> ()
    .AddAttribute ("Local",
                   "The Address on which to Bind the rx socket.",
                   AddressValue (),
                   MakeAddressAccessor (&DashSimpleAttacker::m_local),
                   MakeAddressChecker ())
    .AddAttribute ("Protocol",
                   "The type id of the protocol to use for the rx socket.",
                   TypeIdValue (UdpSocketFactory::GetTypeId ()),
                   MakeTypeIdAccessor (&DashSimpleAttacker::m_tid),
                   MakeTypeIdChecker ())
    .AddAttribute ("NumberOfMiners", 
				   "The number of miners",
                   UintegerValue (16),
                   MakeUintegerAccessor (&DashSimpleAttacker::m_noMiners),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("FixedBlockSize", 
				   "The fixed size of the block",
                   UintegerValue (0),
                   MakeUintegerAccessor (&DashSimpleAttacker::m_fixedBlockSize),
                   MakeUintegerChecker<uint32_t> ())				   
    .AddAttribute ("FixedBlockIntervalGeneration", 
                   "The fixed time to wait between two consecutive block generations",
                   DoubleValue (0),
                   MakeDoubleAccessor (&DashSimpleAttacker::m_fixedBlockTimeGeneration),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("InvTimeoutMinutes", 
				   "The timeout of inv messages in minutes",
                   TimeValue (Minutes (20)),
                   MakeTimeAccessor (&DashSimpleAttacker::m_invTimeoutMinutes),
                   MakeTimeChecker())
    .AddAttribute ("HashRate", 
				   "The hash rate of the simple attacker",
                   DoubleValue (0.2),
                   MakeDoubleAccessor (&DashSimpleAttacker::m_hashRate),
                   MakeDoubleChecker<double> ())	
    .AddAttribute ("BlockGenBinSize", 
				   "The block generation bin size",
                   DoubleValue (-1),
                   MakeDoubleAccessor (&DashSimpleAttacker::m_blockGenBinSize),
                   MakeDoubleChecker<double> ())	
    .AddAttribute ("BlockGenParameter", 
				   "The block generation distribution parameter",
                   DoubleValue (-1),
                   MakeDoubleAccessor (&DashSimpleAttacker::m_blockGenParameter),
                   MakeDoubleChecker<double> ())	
    .AddAttribute ("AverageBlockGenIntervalSeconds", 
				   "The average block generation interval we aim at (in seconds)",
                   DoubleValue (10*60),
                   MakeDoubleAccessor (&DashSimpleAttacker::m_averageBlockGenIntervalSeconds),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("SecureBlocks", 
				   "The number of blocks required for the secure confirmation of the transactions",
                   UintegerValue (6),
                   MakeUintegerAccessor (&DashSimpleAttacker::m_secureBlocks),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("AdvertiseBlocks", 
				   "Choose whether the attacker will advertise his generated blocks",
                   UintegerValue (0),
                   MakeUintegerAccessor (&DashSimpleAttacker::m_advertiseBlocks),
                   MakeUintegerChecker<uint32_t> ())
    .AddTraceSource ("Rx",
                     "A packet has been received",
                     MakeTraceSourceAccessor (&DashSimpleAttacker::m_rxTrace),
...
DashSimpleAttacker::DashSimpleAttacker () : DashMiner(), m_attackFinished(false)
{
  NS_LOG_FUNCTION (this);
}


DashSimpleAttacker::~DashSimpleAttacker(void)
{
  NS_LOG_FUNCTION (this);
}
...
void 
DashSimpleAttacker::StartApplication ()    // Called at time specified by Start
{
...
void 
DashSimpleAttacker::StopApplication ()
{
...
void 
DashSimpleAttacker::DoDispose (void)
{
  //NS_LOG_FUNCTION (this);
  DashMiner::DoDispose ();
...
void 
DashSimpleAttacker::MineBlock (void)  
{
  //NS_LOG_FUNCTION (this);
...
/* 	  //Send large packet
  ...
  //             << "s dash simple attacker " << GetNode ()->GetId () 
...
void 
DashSimpleAttacker::ReceivedHigherBlock(const Block &newBlock) 
{
  //NS_LOG_FUNCTION (this);
  //NS_LOG_INFO("Dash simple attacker "<< GetNode ()->GetId () << " added a new block in the m_blockchain with higher height: " << newBlock);
