...
/*   double minersHash[] = {0.4, 0.4, 0.3};
  ...
  //LogComponentEnable("DashSelfishMiner", LOG_LEVEL_INFO);
  ...
  { 
    ...
    for(auto &miner : miners)
    {
	  ...
	  if (systemId == targetNode->GetSystemId())
	  {
	    ...
	    if (attackerId == miner)
        {
          dashMinerHelper.SetMinerType (SELFISH_MINER);
        }
        ...
        if (test == true && attackerId != miner)
          dashMinerHelper.SetAttribute("FixedBlockIntervalGeneration", DoubleValue(100));
        else if (test == true && attackerId == miner)
          dashMinerHelper.SetAttribute("FixedBlockIntervalGeneration", DoubleValue(500));
